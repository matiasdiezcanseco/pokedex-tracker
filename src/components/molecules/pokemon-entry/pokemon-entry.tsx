...
const PokemonEntry: React.FC<PokedexPokemon> = ({
  ...
  return (
    <div className="border-[1px] border-slate-800 p-4 w-full flex flex-col rounded-lg items-center relative">
      ...
      <Image src={shiny ? shinySprite : sprite} alt={name + ' image'} width={100} height={100} />
      ...
      {caught && (
        <div className="absolute  top-4 right-4">
          <Image src="/assets/pokeball.svg" alt="pokeball" width={20} height={20} />
        </div>
      )}
      {shiny && (
        <div className="absolute  top-4 left-4">
          <Image src="/assets/star.svg" alt="shiny star" width={20} height={20} />
        </div>
      )}
    </div>
  )
